...
var tagTake = {
        ...
        $("#" + inputId + "_tagcontaine .tagList .tagItem").dblclick(function () {
            ...
            updateInputObj.keydown(function (event) {
                if (event.keyCode == 13) {
                    var inputValue = $(this).val();
                    if (inputValue != null && inputValue != "") {
                        tagItemObj.find("span").html($.trim(inputValue));
                        tagItemObj.css("display", "block");
                    } else {
                        tagItemObj.remove();
                    }
    ...
    "resetTagValue": function (inputId) {
        ...
        for (var i = 0; i < tags.length; i++) {
            ...
            var curTag = tags.eq(i).find("span").html();
            tags.eq(i).find('input').val($.trim(curTag));
        }
    ...
    "getTagItemModel": function (valueStr) {
        return '<div class="tagItem"><input name="tags[]" value="' + valueStr + '" type="hidden"><span>' + valueStr + '</span><div class="delete"></div></div>';
    }
}
